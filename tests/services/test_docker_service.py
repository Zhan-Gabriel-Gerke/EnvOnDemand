...
async def test_run_container_success(mock_docker_module, db_session: AsyncSession):
    """
    Tests the successful scenario of running a container.
    Checks that all necessary Docker API methods are called with the correct arguments.
    """
    # --- 1. Arrange ---
    ...
    with patch.object(docker_service, '_find_free_port', return_value=54321) as mock_find_port:
        # --- 2. Act ---
...
@pytest.mark.asyncio
@patch(DOCKER_MODULE_PATH)
async def test_run_container_image_not_found(mock_docker_module, db_session: AsyncSession):
    """
...
@pytest.mark.asyncio
@patch(DOCKER_MODULE_PATH)
async def test_run_container_and_save_to_db(mock_docker_module, db_session: AsyncSession):
    """
    Hybrid test:
    ...
    # --- 2. Act ---
    docker_service = DockerService()
    with patch.object(docker_service, '_find_free_port', return_value=8000):
