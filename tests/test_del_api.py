...
async def test():
    ...
    async with httpx.AsyncClient() as c:
        ...
        # First, query all deployments to get a valid ID just in case
        r = await c.get("http://localhost:8000/api/deployments", headers=headers)
        if r.status_code == 200 and r.json():
            dep_id = r.json()[0]["id"]
            print(f"Deleting deployment: {dep_id}")
            r2 = await c.delete(f"http://localhost:8000/api/deployments/{dep_id}", headers=headers)
